...
export const createOrdersService = (db: Kysely<Database>) => {
  ...
  const create = async (input: OrderInput): Promise<OrderDetail> => {
    ...
    await db.transaction().execute(async (trx) => {
      const receivedStatus = await trx.selectFrom('order_statuses').select('id').where('code', 'in', ['RECEIVED', 'CREATED']).orderBy('id').executeTakeFirstOrThrow();
      ...
      await trx.insertInto('order_status_history').values({ order_id: orderId, status_id: receivedStatus.id, notes: 'Orden recibida' }).execute();
      await trx.insertInto('order_logs').values({ order_id: orderId, event_type: 'CREATE', description: 'Orden creada en escritorio' }).execute();
      await trx.insertInto('audit_logs').values({ action: 'ORDER_CREATE', entity_type: 'order', entity_id: String(orderId), details_json: JSON.stringify({ orderNumber, total, balanceDue }) }).execute();
    });

    if (paidTotal > 0) {
      await paymentsService.create({ orderId, paymentMethodId: 1, amount: paidTotal, reference: 'Abono inicial' });
    }
  ...
  return {
    async dashboard(): Promise<DashboardSummary> {
      const [clients, openOrders, dailySales, pendingBalance, openWarranties, dailyExpenses, recentOrders, paymentBreakdown] = await Promise.all([
        ...
        db.selectFrom('warranties').select((eb) => eb.fn.count<number>('id').as('count')).executeTakeFirst(),
        db.selectFrom('expenses').select((eb) => eb.fn.sum<number>('amount').as('sum')).where('created_at', '>=', new Date(new Date().toDateString())).executeTakeFirst(),
        repository.list(),
        db.selectFrom('payments as p').innerJoin('payment_methods as pm', 'pm.id', 'p.payment_method_id').select([sql<string>`pm.name`.as('method_name'), (eb) => eb.fn.sum<number>('p.amount').as('amount')]).where('p.created_at', '>=', new Date(new Date().toDateString())).groupBy('pm.name').execute()
      ]);
      return {
        ...
        paymentBreakdown: paymentBreakdown.map((item) => ({ methodName: item.method_name, amount: Number(item.amount ?? 0) }))
      };
    },
